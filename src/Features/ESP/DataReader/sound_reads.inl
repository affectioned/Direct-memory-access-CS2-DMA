    ...
    if (g::espSound) {
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            if (s_soundPrevStateValid[i] && soundScatterOk) {
                if (g::espSoundShots && shotsQueued[i] && curShots > s_prevShotsFired[i])
                    PushSoundEvent(SoundEventType::Shot, emitPos, soundNowUs);
                if (g::espSoundReloads && reloadQueued[i] && curInReload && !s_prevInReload[i])
            ...
            if (g::espSoundFootsteps) {
                const float vx = velocities[i].x;
                const float vy = velocities[i].y;
                const float speed2D = std::sqrt(vx * vx + vy * vy);
                if (speed2D >= esp::intervals::kSoundFootstepSpeedThreshold &&
                    (soundNowUs - s_lastFootstepEmitUs[i]) >= esp::intervals::kSoundFootstepEmitIntervalUs) {
