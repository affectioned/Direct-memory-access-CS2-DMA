if (g::espSound) {
    const float durationSec = std::clamp(g::espSoundDuration, 0.25f, 5.0f);
    const float rangeWorld = std::clamp(g::espSoundRange, 100.0f, 6000.0f);
    ...
    for (int idx = 0; idx < eventCount; ++idx) {
        ...
        if (evt.type == SoundEventType::Footstep && !g::espSoundFootsteps) continue;
        if (evt.type == SoundEventType::Shot && !g::espSoundShots) continue;
        if (evt.type == SoundEventType::Reload && !g::espSoundReloads) continue;
