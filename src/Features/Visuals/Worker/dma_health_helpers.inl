...
visuals::DmaHealthStats visuals::GetDmaHealthStats()
{
    const uint64_t nowMs = TickNowUs() / 1000u;
    visuals::DmaHealthStats stats = {};
    ...
    }
    {
        ...
        stats.eventCount = static_cast<int>(std::min<uint32_t>(
            s_dmaEventCount,
            static_cast<uint32_t>(visuals::DmaHealthStats::kMaxEvents)));
        for (int i = 0; i < stats.eventCount; ++i) {
            const uint32_t newestIndex =
                (s_dmaEventWriteIndex + visuals::DmaHealthStats::kMaxEvents - 1u - static_cast<uint32_t>(i)) %
                visuals::DmaHealthStats::kMaxEvents;
    ...
    stats.gameStatus = static_cast<visuals::GameStatus>(s_gameStatus.load(std::memory_order_relaxed));
    return stats;
}

visuals::DebugStats visuals::GetDebugStats()
{
    const uint64_t nowUs = TickNowUs();
    visuals::DebugStats stats = {};
