    ...
    void DataWorkerLoop()
    {
        ...
        while (!s_dataWorkerStopRequested.load(std::memory_order_relaxed)) {
            try {
                ...
                // Periodic data-worker status every 10 s
                static uint64_t s_statusLogUs = 0;
                if (cycleStartUs - s_statusLogUs >= 10000000u) {
                    s_statusLogUs = cycleStartUs;
                    DmaLogPrintf("[INFO] DataWorker: cycle=%lluus failures=%u/%u clientBase=0x%llX engine2=0x%llX signOn=%d inGame=%d",
                        static_cast<unsigned long long>(cycleUs),
                        s_dmaConsecutiveFailures.load(std::memory_order_relaxed),
                        s_dmaTotalFailures.load(std::memory_order_relaxed),
                        static_cast<unsigned long long>(g::clientBase),
                        static_cast<unsigned long long>(g::engine2Base),
                        s_engineSignOnState.load(std::memory_order_relaxed),
                        s_engineInGame.load(std::memory_order_relaxed) ? 1 : 0);
                }
            } catch (...) {
            ...
            bool populationWatchdogRecovery = false;
            {
                ...
                if (zeroPopulationObserved) {
                    ...
                    if (canAct && zeroPopulationStage < 1u && zeroAge >= std::chrono::milliseconds(900)) {
                        ...
                        SetSceneWarmupState(visuals::SceneWarmupState::HierarchyWarming, watchdogNowUs);
                        RefreshDmaCaches(
                    ...
                    if (canAct && zeroPopulationStage < 3u && zeroAge >= std::chrono::milliseconds(5200)) {
                        ...
                        ResetRuntimeState(true);
                        SetSceneWarmupState(
                            suspiciousFlatEntityRange ? visuals::SceneWarmupState::HierarchyWarming : visuals::SceneWarmupState::Recovery,
                            watchdogNowUs);
                        RefreshDmaCaches(
                        ...
                        lastZeroPopulationHardUs = watchdogNowUs;
                        SetSceneWarmupState(visuals::SceneWarmupState::Recovery, watchdogNowUs);
