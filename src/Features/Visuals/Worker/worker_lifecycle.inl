...
void visuals::StartDataWorker()
{
    if (s_dataWorkerRunning.exchange(true, std::memory_order_relaxed)) {
        DmaLogPrintf("[WARN] StartDataWorker: already running, skipping duplicate call");
        return;
    }
    DmaLogPrintf("[INFO] StartDataWorker: launching data worker (%d Hz) and camera worker (%d Hz)",
        DATA_WORKER_HZ, CAMERA_WORKER_HZ);
...
void visuals::StopDataWorker()
{
    DmaLogPrintf("[INFO] StopDataWorker: signaling stop and joining threads");
