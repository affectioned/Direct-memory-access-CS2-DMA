    ...
    for (int i = 0; i < worldMarkerCount; ++i) {
        ...
        if (isDroppedWeapon) {
            ...
            if (marker.weaponId == 49) {
                ...
                if (bombCarriedByAnyPlayer)
                    continue;
                const bool bombVisualsAlreadyVisible =
                    g::visualsBombInfo &&
                    (bombState.planted || bombState.dropped) &&
                    isFiniteVec(bombState.position);
                if (bombVisualsAlreadyVisible)
                    continue;
                if (!g::visualsBombInfo)
                    continue;
            } else {
                if (!g::visualsItem)
                    continue;
            ...
            if (IsKnifeItemId(marker.weaponId))
                continue;
            if (marker.weaponId != 49 && !g::visualsItemEnabledMask.test(marker.weaponId))
                continue;
        } else if (isUtilityEffect) {
            if (!g::visualsWorld)
                continue;
        } else if (isUtilityProjectile) {
            if (!g::visualsWorld || !g::visualsWorldProjectiles)
                continue;
        } else {
            continue;
        }
        ...
        const bool showTimer =
            (marker.type == WorldMarkerType::Smoke && g::visualsWorldSmokeTimer) ||
            (marker.type == WorldMarkerType::Inferno && g::visualsWorldInfernoTimer) ||
            (marker.type == WorldMarkerType::Decoy && g::visualsWorldDecoyTimer) ||
            (marker.type == WorldMarkerType::Explosive && g::visualsWorldExplosiveTimer);
        char markerText[96] = {};
