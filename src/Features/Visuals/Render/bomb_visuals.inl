if (g::visualsEnabled && g::visualsBombInfo && (bombState.planted || bombState.dropped)) {
    ...
    const float bombFontSz = g::visualsBombTextSize > 0.0f ? g::visualsBombTextSize : 0.0f;
    ImFont* bombFont = ImGui::GetFont();
    const float bombFsUse = bombFontSz > 0.0f ? bombFontSz : ImGui::GetFontSize();
    char defText[32] = {};
    char blowText[24] = {};

    if (g::visualsBombText && !g::visualsBombTime && plantedBomb && bombState.beingDefused && defuseLeft > 0.0f) {
    ...
    if (hasScreenBox) {
        ...
        if (g::visualsBombText && !plantedBomb) {
        ...
        if (g::visualsBombText && plantedBomb) {
            ...
            if (!g::visualsBombTime && plantedBomb && bombState.ticking && blowLeft > 0.0f) {
        ...
        if (plantedBomb && bombState.beingDefused && defuseLeft > 0.0f) {
            const float expectedDefuseTime =
                (defuseLeft > visuals::style::kBombDefuseKitHeuristicThreshold)
                    ? visuals::style::kBombDefuseNoKitSeconds
                    : visuals::style::kBombDefuseKitSeconds;
