...
if (g::visualsBox) {
    ...
    DrawCornerBox(drawList, boxLeft, boxTop, boxWidth, boxHeight,
                  entityCol, IM_COL32(0, 0, 0, 220), cornerLen, 2.0f * g::visualsThickness);
}

if (g::visualsBombInfo && p.hasBomb && !bombState.dropped && !bombState.planted) {
    const float pad = 4.0f;
    const ImVec2 minPt(boxLeft - pad, boxTop - pad);
    const ImVec2 maxPt(boxLeft + boxWidth + pad, boxTop + boxHeight + pad);
    const float bombInner = 1.8f * g::visualsThickness;
