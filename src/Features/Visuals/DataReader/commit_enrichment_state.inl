    ...
    static Vector3 s_lastGoodBones[64][visuals::kPlayerStoredBoneCount] = {};

    auto getStoredBoneCompactIndex = [&](int boneId) -> int {
        for (int storedIdx = 0; storedIdx < visuals::kPlayerStoredBoneCount; ++storedIdx) {
            if (visuals::kPlayerStoredBoneIds[storedIdx] == boneId)
                return storedIdx;
        }
        return -1;
    };
    ...
    auto storedBoneLooksPlausible = [&](const Vector3* storedBones, int playerSlot) -> bool {
        ...
        const int pelvisIdx = getStoredBoneCompactIndex(visuals::PELVIS);
        const int chestIdx = getStoredBoneCompactIndex(visuals::CHEST);
        const int spine2Idx = getStoredBoneCompactIndex(visuals::SPINE2);
        const int headIdx = getStoredBoneCompactIndex(visuals::HEAD);
        const int leftHeelIdx = getStoredBoneCompactIndex(visuals::FOOT_HEEL_L);
        const int rightHeelIdx = getStoredBoneCompactIndex(visuals::FOOT_HEEL_R);
    ...
    auto hasPlausibleCommittedBones = [&](int playerSlot) -> bool {
        ...
        Vector3 currentStoredBones[visuals::kPlayerStoredBoneCount] = {};
        for (int storedIdx = 0; storedIdx < visuals::kPlayerStoredBoneCount; ++storedIdx)
            currentStoredBones[storedIdx] = allBones[playerSlot][visuals::kPlayerStoredBoneIds[storedIdx]];
    ...
    auto rememberGoodCommittedBones = [&](int playerSlot, const Vector3* bones) {
        ...
        for (int boneIdx = 0; boneIdx < visuals::kPlayerStoredBoneCount; ++boneIdx)
    ...
    auto canReusePreviousCommittedBones = [&](int playerSlot) -> bool {
        ...
        const visuals::PlayerData& prevPlayer = s_prevPlayers[playerSlot];
    ...
    auto copyResolvedBones = [&](int playerSlot, visuals::PlayerData& targetPlayer) -> bool {
        if (hasPlausibleCommittedBones(playerSlot)) {
            targetPlayer.hasBones = true;
            for (int boneIdx = 0; boneIdx < visuals::kPlayerStoredBoneCount; ++boneIdx)
                targetPlayer.bones[boneIdx] = allBones[playerSlot][visuals::kPlayerStoredBoneIds[boneIdx]];
    ...
    auto collectGrenadesForSlot = [&](int idx, auto& grenadeIdsOut, int& grenadeCountOut) {
        ...
        auto appendGrenade = [&](uint16_t weaponId) {
            if (weaponId < 43u || weaponId > 48u)
                return;
            if (grenadeCountOut >= visuals::PlayerData::kMaxGrenades)
                return;
            grenadeIdsOut[grenadeCountOut++] = weaponId;
        };

        const int inventorySlotCount = getInventorySlotCount(idx);
        for (int slot = 0; slot < inventorySlotCount && grenadeCountOut < visuals::PlayerData::kMaxGrenades; ++slot)
    ...
    uint16_t localGrenadeIdsResolved[visuals::PlayerData::kMaxGrenades] = {};
    ...
    
    {
...

        {
            ...
            const bool healthyPopulationSample =
                liveMatchContext &&
                populationWarmupState == visuals::SceneWarmupState::Stable &&
            ...
            if (launchUnderresolved) {
                ...
                const uint64_t underresolvedAgeUs =
                    ...
                    ? nowUs - s_launchUnderresolvedSinceUs
                    : 0;
                if (populationWarmupState == visuals::SceneWarmupState::Stable)
    ...
    if (webRadarDemandActive) {
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            if ((!coreReliable || !teamValid || !haveWebRadarPos) && cachedSamePawn) {
                visuals::PlayerData& cached = s_webRadarPlayers[i];
            ...
            visuals::PlayerData& wp = s_webRadarPlayers[i];
            wp.valid = true;
            wp.pawn = pawns[i];
            wp.staleFrames = 0;
