    ...
    auto logStatusTransition = [&](visuals::GameStatus from, visuals::GameStatus to, const char* reason) {
        if (from == to)
            return;
        DmaLogPrintf(
    ...
    auto setSceneWarmupState = [&](visuals::SceneWarmupState state) {
    ...
    auto enterWaitCs2 = [&]() {
        const auto prevStatus = static_cast<visuals::GameStatus>(s_gameStatus.load(std::memory_order_relaxed));
        const uint64_t nowUs = TickNowUs();
        const auto warmupState = static_cast<visuals::SceneWarmupState>(s_sceneWarmupState.load(std::memory_order_relaxed));
        const bool recentColdAttachReset =
            prevStatus == visuals::GameStatus::WaitCs2 &&
            warmupState == visuals::SceneWarmupState::ColdAttach &&
        ...
        s_gameStatus.store(static_cast<uint8_t>(visuals::GameStatus::WaitCs2), std::memory_order_relaxed);
        logStatusTransition(prevStatus, visuals::GameStatus::WaitCs2, "client_base_lost_or_dma_recovery");
        if (!recentColdAttachReset)
            clearAllState(true);
        s_mapFingerprint = 0;
        setSceneWarmupState(visuals::SceneWarmupState::ColdAttach);
        g::clientBase = 0;
        g::engine2Base = 0;
        MarkDmaReadSuccess();
    };
    ...
    auto promoteInGameFrame = [&](const char* reason) {
        ...
        if (curStatus != visuals::GameStatus::Ok) {
            s_gameStatus.store(static_cast<uint8_t>(visuals::GameStatus::Ok), std::memory_order_relaxed);
            logStatusTransition(curStatus, visuals::GameStatus::Ok, reason);
        }
    };
