        ...
        auto sceneWarmupState =
            static_cast<visuals::SceneWarmupState>(s_sceneWarmupState.load(std::memory_order_relaxed));
        const bool hierarchyWarmupActive =
            !s_controllerCacheWarmed ||
            sceneSettling ||
            sceneWarmupState != visuals::SceneWarmupState::Stable;
        ...
        if (hierarchyRefreshDue && playerRefreshSlotCount > 0) {
            ...
            // Fallback: if primary stride (kEntitySlotSize) found no controllers,
            // retry with kEntitySlotSizeFallback (0x78). CS2 changed the stride
            // between versions; 0x78 is what most working cheats use.
            if (kEntitySlotSizeFallback != kEntitySlotSize && listEntry) {
                bool anyControllerPrimary = false;
                for (int slotIdx = 0; slotIdx < playerRefreshSlotCount; ++slotIdx) {
                    if (controllers[playerRefreshSlots[slotIdx]]) { anyControllerPrimary = true; break; }
                }
                if (!anyControllerPrimary) {
                    static uint32_t s_fallbackAttempts = 0;
                    static uint32_t s_fallbackHits = 0;
                    ++s_fallbackAttempts;
                    uintptr_t fallbackControllers[64] = {};
                    for (int slotIdx = 0; slotIdx < playerRefreshSlotCount; ++slotIdx) {
                        const int i = playerRefreshSlots[slotIdx];
                        const int controllerSlot = i + 1;
                        mem.AddScatterReadRequest(handle,
                            listEntry + kEntitySlotSizeFallback * (controllerSlot & kEntitySlotMask),
                            &fallbackControllers[i], sizeof(uintptr_t));
                    }
                    executeOptionalScatterRead();
                    bool anyFallback = false;
                    for (int slotIdx = 0; slotIdx < playerRefreshSlotCount; ++slotIdx) {
                        const int i = playerRefreshSlots[slotIdx];
                        if (isLikelyGamePointer(fallbackControllers[i])) {
                            controllers[i] = fallbackControllers[i];
                            anyFallback = true;
                        }
                    }
                    if (anyFallback) {
                        ++s_fallbackHits;
                        if (s_fallbackHits <= 3 || (s_fallbackHits % 30u) == 0u)
                            DmaLogPrintf("[WARN] ControllerScan: primary stride 0x%X found nothing, fallback stride 0x%X found controllers (hits=%u listEntry=0x%llX)",
                                kEntitySlotSize, kEntitySlotSizeFallback, s_fallbackHits,
                                static_cast<unsigned long long>(listEntry));
                    } else if (s_fallbackAttempts <= 3 || (s_fallbackAttempts % 60u) == 0u) {
                        DmaLogPrintf("[INFO] ControllerScan: both strides 0x%X and 0x%X found nothing (listEntry=0x%llX attempts=%u)",
                            kEntitySlotSize, kEntitySlotSizeFallback,
                            static_cast<unsigned long long>(listEntry), s_fallbackAttempts);
                    }
                }
            }
                    ...
                    if (!sceneSettling && liveHierarchyExpected) {
                        ...
                        if (refreshCooldownElapsed && s_zeroControllerStreak >= 4) {
                            ...
                            if (s_zeroControllerStreak >= 36) {
                                setSceneWarmupState(visuals::SceneWarmupState::Recovery);
                                sceneWarmupState = visuals::SceneWarmupState::Recovery;
...

            {
                ...
                const uint64_t missingHoldUs =
                    recentStructuralReset || sceneWarmupState != visuals::SceneWarmupState::Stable
...

            {
                ...
                if (!s_controllerCacheWarmed) {
                    if (hierarchyWarmupSatisfied) {
                        ...
                        if (sceneWarmupState != visuals::SceneWarmupState::Stable) {
                            setSceneWarmupState(visuals::SceneWarmupState::Stable);
                            sceneWarmupState = visuals::SceneWarmupState::Stable;
                        }
                    } else if (sceneWarmupState != visuals::SceneWarmupState::SceneTransition) {
                        setSceneWarmupState(visuals::SceneWarmupState::HierarchyWarming);
                        sceneWarmupState = visuals::SceneWarmupState::HierarchyWarming;
                    }
                }
