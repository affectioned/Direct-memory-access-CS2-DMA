    ...
    const uint64_t fullInventoryLaneIntervalUs =
        webRadarDemandActive
        ? std::min<uint64_t>(visuals::intervals::kInventoryFullInventoryLaneUs, 20000u)
        : visuals::intervals::kInventoryFullInventoryLaneUs;
    bool noKnifePrimaryInventoryRefreshNeeded = false;
    if (wantsInventoryData && g::visualsWeapon && g::visualsWeaponIcon && g::visualsWeaponIconNoKnife) {
        for (int inventorySlotIdx = 0; inventorySlotIdx < inventoryPlayerSlotCount; ++inventorySlotIdx) {
    ...
    const bool activeWeaponLaneDue =
        wantsInventoryData &&
        (s_lastActiveWeaponLaneUs == 0 ||
         (inventoryNowUs - s_lastActiveWeaponLaneUs) >= visuals::intervals::kInventoryActiveWeaponLaneUs);
