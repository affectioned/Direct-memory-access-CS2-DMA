        ...
        for (int processIdx = 0; processIdx < worldProcessCount; ++processIdx) {
            ...
            const bool droppedItemCandidate =
                ...
                knownDroppedItem &&
                itemId < 1200 &&
                g::visualsItemEnabledMask.test(itemId) &&
        ...
        if (kDebugWorldUtility) {
            ...
            if ((s_dbgScan % 12u) == 0u || anomaly) {
                DmaLogPrintf(
                    "[DEBUG] WorldVisuals: scan=%u time=%.2f interval=%.5f limit=%d markers=%d evidence=%d raw(S/I/D/E)=%d/%d/%d/%d signal=%d/%d/%d/%d anomaly=%d",
                    s_dbgScan,
                    currentGameTime,
                    safeIntervalPerTick,
                    worldLimit,
                ...
                for (int i = 0; i < dbgNoSignalSamples; ++i) {
                    ...
                    DmaLogPrintf(
                        "[DEBUG] WorldVisuals sample: idx=%d item=%u owner=%u ticks(s/i/d/d2/e)=%d/%d/%d/%d/%d fire=%d active=%d rem(s/i/d/e)=%.2f/%.2f/%.2f/%.2f",
                        s.idx,
