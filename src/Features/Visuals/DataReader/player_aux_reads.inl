    ...
    const bool wantsPlayerMoneyAux =
        webRadarDemandActive ||
        (g::visualsFlags && g::visualsFlagMoney);
    const bool wantsPlayerStatusAux =
        webRadarDemandActive ||
        (g::visualsFlags && (g::visualsFlagScoped || g::visualsFlagDefusing || g::visualsFlagBlind));
    const bool wantsPlayerDefuserAux =
        webRadarDemandActive ||
        (g::visualsFlags && g::visualsFlagKit);
    ...
    const bool playerIdentityAuxDue =
        ...
        (s_lastPlayerIdentityAuxUs == 0 ||
         (playerAuxNowUs - s_lastPlayerIdentityAuxUs) >= visuals::intervals::kPlayerIdentityAuxUs);
    const bool playerMoneyAuxDue =
        wantsPlayerMoneyAux &&
        (s_lastPlayerMoneyAuxUs == 0 ||
         (playerAuxNowUs - s_lastPlayerMoneyAuxUs) >= visuals::intervals::kPlayerMoneyAuxUs);
    const bool playerStatusAuxDue =
        wantsPlayerStatusAux &&
        (s_lastPlayerStatusAuxUs == 0 ||
         (playerAuxNowUs - s_lastPlayerStatusAuxUs) >= visuals::intervals::kPlayerStatusAuxUs);
    const bool playerDefuserAuxDue =
        wantsPlayerDefuserAux &&
        (s_lastPlayerDefuserAuxUs == 0 ||
         (playerAuxNowUs - s_lastPlayerDefuserAuxUs) >= visuals::intervals::kPlayerDefuserAuxUs);
    const bool playerEyeAuxDue =
        wantsPlayerEyeAux &&
        (s_lastPlayerEyeAuxUs == 0 ||
         (playerAuxNowUs - s_lastPlayerEyeAuxUs) >= visuals::intervals::kPlayerEyeAuxUs);
    const bool playerVisibilityAuxDue =
        wantsPlayerVisibilityAux &&
        hasSpottedStateOffsets &&
        (s_lastPlayerVisibilityAuxUs == 0 ||
         (playerAuxNowUs - s_lastPlayerVisibilityAuxUs) >= visuals::intervals::kPlayerVisibilityAuxUs);
    const bool playerSpectatorAuxDue =
        wantsPlayerSpectatorAux &&
        (s_lastPlayerSpectatorAuxUs == 0 ||
         (playerAuxNowUs - s_lastPlayerSpectatorAuxUs) >= visuals::intervals::kPlayerSpectatorAuxUs);
