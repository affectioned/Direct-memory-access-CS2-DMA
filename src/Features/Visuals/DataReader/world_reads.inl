    ...
    const bool wantsDroppedItemMarkers = g::visualsItem;
    ...
    const bool wantsWorldUtilityMarkers = g::visualsWorld || webRadarDemandActive;
    const bool wantsWorldUtilityData = wantsWorldUtilityMarkers || wantsWorldProjectiles;
    const bool wantsBombConsumers = g::visualsBombInfo || g::radarShowBomb || webRadarDemandActive;
    ...
    if (shouldScanWorld && entityList && ofs.CGameSceneNode_m_vecAbsOrigin > 0 && highestEntityIndex > 64) {
        ...
        if ((s_dbgSkipScan % 24u) == 0u) {
            const bool modeEnabled = (g::visualsWorld || g::visualsBombInfo || g::radarShowBomb || webRadarDemandActive);
            const uint64_t sinceLast = nowUs - s_lastWorldScanUs;
            DmaLogPrintf(
                "[DEBUG] WorldVisuals skipped: mode=%d gate=%d since=%llu entityList=%d posOfs=%d highest=%d shouldScan=%d",
                modeEnabled ? 1 : 0,
