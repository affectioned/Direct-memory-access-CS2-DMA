    ...
    if (g::visualsSkeleton) {
        ...
        auto clearResolvedBoneSlot = [&](int i) {
            hasBoneData[i] = false;
            sceneNodes[i] = 0;
            for (int b = 0; b < visuals::kPlayerStoredBoneCount; ++b) {
                const int boneIdx = visuals::kPlayerStoredBoneIds[b];
                allBones[i][boneIdx] = {};
            }
        };
        auto copyCommittedBoneSlot = [&](int i, const visuals::PlayerData& committed) -> bool {
            if (i < 0 || i >= 64)
                return false;
            if (!committed.valid ||
                !committed.hasBones ||
        ...
        auto hasReusableCommittedBoneSlot = [&](int i) -> bool {
            if (i < 0 || i >= 64 || !pawns[i])
                return false;
            const visuals::PlayerData& current = s_players[i];
            if (current.valid && current.hasBones && current.pawn == pawns[i])
                return true;
            const visuals::PlayerData& previous = s_prevPlayers[i];
        ...
        const bool boneReadsDue =
            ...
            (boneNowUs - s_lastBoneReadsUs) >= visuals::intervals::kBoneReadsUs;
            ...
            if (narrowDebugEnabled(kNarrowDebugBones)) {
                ...
                if (emitBonesDebug) {
                    ...
                    DmaLogPrintf(
                        ...
                        boneArrayFailures,
                        boneReadFailures,
                        g::visualsSkeleton ? 1 : 0,
                        sceneNodeCount,
                        boneArrayCount,
                        readyCount,
                        visuals::kPlayerStoredBoneCount);
                }
            }
