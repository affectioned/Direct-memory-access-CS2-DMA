    ...
    }
    {
        ...
        const bool highestEntityRefreshDue =
            s_lastHighestEntityRefreshUs == 0 ||
            (baseNowUs - s_lastHighestEntityRefreshUs) >= visuals::intervals::kBaseHighestEntityRefreshUs;
        const bool minimapRefreshDue =
            !s_hasMinimapBounds ||
            s_lastMinimapRefreshUs == 0 ||
            (baseNowUs - s_lastMinimapRefreshUs) >= visuals::intervals::kBaseMinimapRefreshUs;
        const bool sensitivityRefreshDue =
            s_sensitivity <= 0.0f ||
            s_lastSensitivityRefreshUs == 0 ||
            (baseNowUs - s_lastSensitivityRefreshUs) >= visuals::intervals::kBaseSensitivityRefreshUs;
        const bool intervalRefreshDue =
            s_lastStableIntervalPerTick <= 0.0f ||
            s_lastIntervalRefreshUs == 0 ||
            (baseNowUs - s_lastIntervalRefreshUs) >= visuals::intervals::kBaseIntervalRefreshUs;
        const bool liveMapNameRefreshDue =
            s_lastLiveMapNameRefreshUs == 0 ||
            (baseNowUs - s_lastLiveMapNameRefreshUs) >= visuals::intervals::kBaseMinimapRefreshUs;
        ...
        // Offset + raw-read diagnostic — logs once then every 30 s to confirm offset health
        {
            static uint64_t s_offsetDiagLastUs = 0;
            const uint64_t offsetNowUs = TickNowUs();
            if (offsetNowUs - s_offsetDiagLastUs >= 30000000u) {
                s_offsetDiagLastUs = offsetNowUs;
                DmaLogPrintf(
                    "[INFO] OffsetCheck: dwEntityList=0x%llX raw=0x%llX | dwLocalPlayerPawn=0x%llX raw=0x%llX | dwViewMatrix=0x%llX | clientBase=0x%llX",
                    static_cast<unsigned long long>(ofs.dwEntityList),
                    static_cast<unsigned long long>(rawEntityList),
                    static_cast<unsigned long long>(ofs.dwLocalPlayerPawn),
                    static_cast<unsigned long long>(rawLocalPawn),
                    static_cast<unsigned long long>(ofs.dwViewMatrix),
        ...
        {
            static uint64_t s_hierDiagLastUs = 0;
            const uint64_t hierDiagNow = TickNowUs();
            if (hierDiagNow - s_hierDiagLastUs >= 10000000u) {
                s_hierDiagLastUs = hierDiagNow;
                DmaLogPrintf(
                    "[INFO] HierarchyPtrs: entityList=0x%llX rawListEntry=0x%llX listEntry=0x%llX localPawn=0x%llX",
                    static_cast<unsigned long long>(entityList),
                    static_cast<unsigned long long>(rawListEntry),
        ...
        const bool stableInGameGameRulesWindow =
            s_engineInGame.load(std::memory_order_relaxed) &&
            gameRules != 0 &&
            gameRulesWarmupState == visuals::SceneWarmupState::Stable &&
    ...
    if (entityList && s_cachedEntityList &&
        ...
        const bool stableInGameEntityList =
            ...
            entityListWarmupState == visuals::SceneWarmupState::Stable;
    ...
    const bool sceneSettling =
        ...
        (baseReadsSceneAge < 10000000 &&
         baseWarmupState != visuals::SceneWarmupState::Stable &&
         baseWarmupState != visuals::SceneWarmupState::Recovery);
    {
        ...
        if (!entityHierarchyReady) {
            if (!liveEntityHierarchyContext) {
                ...
                s_entityHierarchyFlatStreak = 0;
                s_lastEntityHierarchyFlatRecoveryUs = 0;
            } else if (baseWarmupState == visuals::SceneWarmupState::Recovery ||
            ...
            if (flatLiveEntityHierarchy &&
                ...
                if (flatCooldownElapsed && s_entityHierarchyFlatStreak >= 3u) {
                    ...
                    if (repairFlat)
                        SetSceneWarmupState(visuals::SceneWarmupState::HierarchyWarming, nowUs);
                    refreshDmaCaches(
