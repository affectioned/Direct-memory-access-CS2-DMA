...
void visuals::Draw()
{
    if (!g::visualsEnabled && !g::radarEnabled && !g::radarSpectatorList && !g::visualsBombTime && !g::visualsSound) {
        static uint64_t s_drawSkipLogUs = 0;
        const uint64_t drawSkipNow = TickNowUs();
        if (drawSkipNow - s_drawSkipLogUs >= 5000000u) {
            s_drawSkipLogUs = drawSkipNow;
            DmaLogPrintf("[WARN] Visuals::Draw: all features off (visuals=%d radar=%d bomb=%d sound=%d)",
                g::visualsEnabled?1:0, g::radarEnabled?1:0, g::visualsBombTime?1:0, g::visualsSound?1:0);
        }
        return;
    }
    ...
    if (screenW <= 0 || screenH <= 0) {
        static uint64_t s_screenLogUs = 0;
        const uint64_t screenNow = TickNowUs();
        if (screenNow - s_screenLogUs >= 5000000u) {
            s_screenLogUs = screenNow;
            DmaLogPrintf("[WARN] Visuals::Draw: invalid screen (%.0fx%.0f)", screenW, screenH);
        }
        return;
    }
    ...
    const visuals::PlayerData* players = snap.players;
    const visuals::PlayerData* prevPlayers = snap.prevPlayers;
    ...
    if (!drawList) {
        DmaLogPrintf("[WARN] Visuals::Draw: no ImGui background draw list");
        return;
    }

    const bool usableViewMatrix = isLikelyViewMatrix(viewMatrix);

    // Periodic active-draw diagnostic every 5 s
    {
        static uint64_t s_drawActiveLogUs = 0;
        if (nowUs - s_drawActiveLogUs >= 5000000u) {
            s_drawActiveLogUs = nowUs;
            int validCount = 0;
            for (int i = 0; i < 64; ++i)
                if (players[i].valid) ++validCount;
            DmaLogPrintf("[INFO] Visuals::Draw: active players=%d viewOk=%d livePos=%d snap=%llu",
                validCount, usableViewMatrix?1:0, usingLiveLocalPos?1:0,
                static_cast<unsigned long long>(snap.captureTimeUs));
        }
    }
    ...
    ImU32 boxCol = ColorToImU32(g::visualsBoxColor);
    ImU32 armorCol = ColorToImU32(g::visualsArmorColor);
    ImU32 nameCol = ColorToImU32(g::visualsNameColor);
    ImU32 distanceCol = ColorToImU32(g::visualsDistanceColor);
    ImU32 visibleCol = ColorToImU32(g::visualsVisibleColor);
    ImU32 hiddenCol = ColorToImU32(g::visualsHiddenColor);
    ImU32 skelCol = ColorToImU32(g::visualsSkeletonColor);
    ImU32 snapCol = ColorToImU32(g::visualsSnaplineColor);
    ImU32 offscreenCol = ColorToImU32(g::visualsOffscreenColor);
    ImU32 worldCol = ColorToImU32(g::visualsWorldColor);
    ImU32 bombCol = ColorToImU32(g::visualsBombColor);
    auto getPlayerBone = [&](const visuals::PlayerData& player, int boneId) -> Vector3 {
        const int idx = visuals::PlayerStoredBoneIndex(boneId);
        if (idx < 0)
            return {};
        return player.bones[idx];
    };
    auto hasUsableBone = [&](const visuals::PlayerData& player, int boneId) -> bool {
    ...
    auto selectToeBoneId = [&](const visuals::PlayerData& player, bool leftSide) -> int {
        const int primary = leftSide
            ? visuals::LeftToeBoneForTeam(player.team)
            : visuals::RightToeBoneForTeam(player.team);
        const int alternate = leftSide
            ? (primary == visuals::FOOT_TOES_L_CT ? visuals::FOOT_TOES_L_T : visuals::FOOT_TOES_L_CT)
            : (primary == visuals::FOOT_TOES_R_CT ? visuals::FOOT_TOES_R_T : visuals::FOOT_TOES_R_CT);
    ...
    if (g::visualsEnabled && usableViewMatrix) {
        for (int i = 0; i < 64; i++) {
            ...
            Vector3 feetPos;
            Vector3 headPos;
            ScreenPos boneScreen[visuals::kSkeletonScreenBoneCapacity] = {};
            bool boneScreenValid[visuals::kSkeletonScreenBoneCapacity] = {};
            const int leftToeBoneId = selectToeBoneId(p, true);
            const int rightToeBoneId = selectToeBoneId(p, false);
            const Vector3 headBone = getPlayerBone(p, visuals::HEAD);
            const Vector3 pelvisBone = getPlayerBone(p, visuals::PELVIS);
            const Vector3 chestBone = getPlayerBone(p, visuals::CHEST);
            const Vector3 leftHeelBone = getPlayerBone(p, visuals::FOOT_HEEL_L);
            const Vector3 rightHeelBone = getPlayerBone(p, visuals::FOOT_HEEL_R);
            ...
            const bool hasHeadBone = hasUsableBone(p, visuals::HEAD);
            const bool hasPelvisBone = hasUsableBone(p, visuals::PELVIS);
            const bool hasChestBone = hasUsableBone(p, visuals::CHEST);
            const bool hasLeftHeelBone = hasUsableBone(p, visuals::FOOT_HEEL_L);
            const bool hasRightHeelBone = hasUsableBone(p, visuals::FOOT_HEEL_R);
            ...
            const bool hasCoreBoneChain =
                hasHeadBone &&
                hasPelvisBone &&
                (hasChestBone || hasUsableBone(p, visuals::SPINE2)) &&
                (hasLeftFootBone || hasRightFootBone);
            int validStoredBoneCount = 0;
            for (int bIdx = 0; bIdx < visuals::kPlayerStoredBoneCount; ++bIdx) {
                if (hasUsableBone(p, visuals::kPlayerStoredBoneIds[bIdx]))
            ...
            if (renderHasReliableBones) {
                ...
                if (canBlendSnapshots && prevP.hasBones)
                    headPos = lerpVec3(getPlayerBone(prevP, visuals::HEAD), headBone);
                else
                ...
                if (canBlendSnapshots && prevP.hasBones) {
                    const Vector3 prevLeftFoot = getPlayerBone(prevP, visuals::FOOT_HEEL_L);
                    const Vector3 prevRightFoot = getPlayerBone(prevP, visuals::FOOT_HEEL_R);
                ...
                for (int bIdx = 0; bIdx < visuals::kPlayerStoredBoneCount; ++bIdx) {
                    const int b = visuals::kPlayerStoredBoneIds[bIdx];
                ...
                if (boneScreenValid[visuals::FOOT_HEEL_L] && hasLeftToeBone && boneScreenValid[leftToeBoneId])
                    ++validBoneSegmentCount;
                if (boneScreenValid[visuals::FOOT_HEEL_R] && hasRightToeBone && boneScreenValid[rightToeBoneId])
                    ++validBoneSegmentCount;
            } else {
                feetPos = renderPlayerPos;
                headPos = feetPos;
...
#include "../Render/player_offscreen_arrows.inl"
            ...
            const ImU32 entityCol = g::visualsVisibilityColoring ? (isVisibleNow ? visibleCol : hiddenCol) : boxCol;
...
#include "../Render/player_snaplines.inl"
    ...
    if (usableViewMatrix) {
#include "../Render/world_visuals.inl"

#include "../Render/bomb_visuals.inl"
