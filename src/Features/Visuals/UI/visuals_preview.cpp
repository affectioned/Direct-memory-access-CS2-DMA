...
namespace
{
    ...
    Vec2 BonePos2D(int boneId, float cx, float top, float scale)
    {
        float s = scale;
        switch (boneId) {
        case visuals::HEAD:           return { cx,          top + 10*s  };
        case visuals::NECK:           return { cx,          top + 22*s  };
        case visuals::CHEST:          return { cx,          top + 38*s  };
        case visuals::SPINE2:         return { cx,          top + 54*s  };
        case visuals::SPINE1:         return { cx,          top + 70*s  };
        case visuals::PELVIS:         return { cx,          top + 90*s  };
        case visuals::SHOULDER_L:     return { cx + 18*s,   top + 26*s  };
        case visuals::ELBOW_L:        return { cx + 32*s,   top + 52*s  };
        case visuals::HAND_L:         return { cx + 34*s,   top + 70*s  };
        case visuals::SHOULDER_R:     return { cx - 18*s,   top + 26*s  };
        case visuals::ELBOW_R:        return { cx - 32*s,   top + 52*s  };
        case visuals::HAND_R:         return { cx - 34*s,   top + 70*s  };
        case visuals::HIP_L:          return { cx + 10*s,   top + 95*s  };
        case visuals::KNEE_L:         return { cx + 13*s,   top + 125*s };
        case visuals::FOOT_HEEL_L:    return { cx + 14*s,   top + 146*s };
        case visuals::FOOT_TOES_L_T:
        case visuals::FOOT_TOES_L_CT: return { cx + 16*s,   top + 154*s };
        case visuals::HIP_R:          return { cx - 10*s,   top + 95*s  };
        case visuals::KNEE_R:         return { cx - 13*s,   top + 125*s };
        case visuals::FOOT_HEEL_R:    return { cx - 14*s,   top + 146*s };
        case visuals::FOOT_TOES_R_T:
        case visuals::FOOT_TOES_R_CT: return { cx - 16*s,   top + 154*s };
    ...
    static const int kJoints[] = {
        ...
        visuals::KNEE_L,
        visuals::FOOT_HEEL_L,
        visuals::FOOT_TOES_L_CT,
        visuals::HIP_R,
        visuals::KNEE_R,
        visuals::FOOT_HEEL_R,
        visuals::FOOT_TOES_R_CT
    };
                              ...
                              float areaBottom, bool useVisColors = true)
    {
        ...
        if (g::visualsSnaplines && showBottomLabels) {
            float fromY = g::visualsSnaplineFromTop ? (boxTop - 30) : areaBottom;
            dl->AddLine(ImVec2(cx, fromY), ImVec2(cx, boxTop + boxH),
                        IM_COL32(0, 0, 0, 180), 2.5f);
            dl->AddLine(ImVec2(cx, fromY), ImVec2(cx, boxTop + boxH),
                        Col4(g::visualsSnaplineColor), 1.0f);
        }

        
        if (g::visualsBox)
            PrvCornerBox(dl, boxLeft, boxTop, boxW, boxH,
                         entityCol, boxH * 0.22f, 2.0f * g::visualsThickness);

        
        const float healthBarLeft = boxLeft - sideBarW - sideBarGap;
        const float armorBarLeft = g::visualsHealth
        ...
        if (g::visualsArmor) {
            PrvSideBar(dl, armorBarLeft, boxTop, boxH, sideBarW, apFrac,
                       Col4(g::visualsArmorColor));
            if (g::visualsArmorText && mockArmor < 100)
                makeBarLabel(apLabel, mockArmor, centeredArmorBarLeft, IM_COL32(225, 245, 255, 255), Col4(g::visualsArmorColor));
        }
        if (g::visualsHealth) {
            PrvSideBar(dl, healthBarLeft, boxTop, boxH, sideBarW, hpFrac,
                       Col4(g::visualsHealthColor));
            if (g::visualsHealthText && mockHp < 100)
                makeBarLabel(hpLabel, mockHp, centeredHealthBarLeft, IM_COL32(255, 255, 255, 255), Col4(g::visualsHealthColor));
        }
        ...
        if (g::visualsSkeleton) {
            ImU32 skelCol = (g::visualsVisibilityColoring && useVisColors) ? entityCol : Col4(g::visualsSkeletonColor);
            const float prvSkelInner = 1.6f * g::visualsThickness;
        ...
        
        {
            ...
            if (g::visualsWeapon) {
                ...
                if (g::fontWeaponIcons && g::visualsWeaponIcon) {
                    const char* icon = "W"; 
                    const float iconSize = g::visualsWeaponIconSize;
                    ...
                    TextShadowFont(dl, g::fontWeaponIcons, iconSize,
                                   ...
                                   Col4(g::visualsWeaponIconColor), icon);
                    bottomY += its.y + 1;
                }
                ...
                if (g::visualsWeaponText) {
                    ...
                    TextShadowFont(dl, tf, tfs, ImVec2(cx - ts.x * 0.5f, bottomY),
                                   Col4(g::visualsWeaponTextColor), weapon);
                    bottomY += ts.y + 2;
                }
            }
            if (g::visualsWeaponAmmo) {
                ...
                TextShadowFont(dl, af, afs, ImVec2(cx - ts.x * 0.5f, bottomY),
                               Col4(g::visualsWeaponAmmoColor), ammo);
            }
        }
...
void ui::RenderVisualsPreview()
{
    if (!g::visualsPreviewOpen || !g::visualsEnabled)
        return;

    const bool triMode = g::visualsVisibilityColoring;
    ...
    if (!ImGui::Begin("Visuals Preview", &g::visualsPreviewOpen,
    ...
    if (triMode) {
        ...
        const char* hidLabel = g::visualsVisibleOnly ? "Hidden (off)" : "Hidden";
        ...
        const ImU32 hidLabelCol = g::visualsVisibleOnly ? IM_COL32(110, 110, 110, 220) : hidCol;
        ...
        ImU32 entityCol = Col4(g::visualsBoxColor);
        ...
        if (g::visualsOffscreenArrows) {
            ImU32 arrowCol = Col4(g::visualsOffscreenColor);
